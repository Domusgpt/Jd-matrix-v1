...
- Set `static_site_generator: none` in the Pages workflow so the managed Pages job skips the legacy Jekyll builder and directly publishes the uploaded Vite bundle.
- Testing: `API_KEY=dummy npm run build`; `API_KEY=dummy npm run test:e2e` (config change only).
