...
- **Pages workflow:** `.github/workflows/pages.yml` builds with a GitHub Pages base path, runs the same smoke tests, and publishes the `dist` output to GitHub Pages. The workflow sets `VITE_BASE_PATH` to `/${{ github.event.repository.name }}/` so assets resolve when served from a subpath and copies `404.html` for SPA routing.
...
- **Local parity:** the same `firebase.json`/`firestore.rules` used in CI are checked in. To dry-run locally, build with `API_KEY` set, then run `firebase emulators:start` or `firebase deploy --only hosting,firestore --project <projectId>`. For GitHub Pages parity, set `VITE_BASE_PATH=/your-repo-name/` before running `npm run build` so assets load correctly from a subpath and copy `dist/index.html` to `dist/404.html` (handled automatically by `postbuild`).
