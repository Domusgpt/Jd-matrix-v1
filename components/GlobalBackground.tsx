...
export const GlobalBackground: React.FC<Props> = ({ appState }) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      if (visualizerRef.current) {
        ...
        if (appState.step === AppStep.MATRIX) {
            // If no image is uploaded yet, use the "Assets" style (Cyan, generic)
            if (!appState.imagePreviewUrl) {
                ...
                // "Director" mode (Image loaded) - use selected style params
        ...
        else if (appState.step === AppStep.PLAYER) {
        ...
        // Audio Reactivity (Idle Mode - Only in Matrix)
        if (appState.step === AppStep.MATRIX) {
  ...
  }, [appState.step, appState.selectedStyleId, appState.secondaryStyleId, appState.morphIntensity, appState.isGenerating, appState.reactivity, appState.imagePreviewUrl]);
